...
별도의 백엔드 서버나 PaperGlint 계정 없이 동작하며, 외부 출처 조회는 사용자가 기능을 열거나 요청했을 때만 시작합니다.
...
PaperGlint는 `Identity`, `Decision`, `Track`을 독립적으로 표시합니다. 검색 후보가 발견됐다는 이유만으로 현재 논문의 게재 상태를 자동 확정하지 않습니다.
