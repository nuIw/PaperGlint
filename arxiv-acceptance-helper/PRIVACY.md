...
PaperGlint는 arXiv 논문 페이지에서 학회 게재 근거, 코드 및 프로젝트 링크,
사용자 지정 PDF 파일명을 제공하는 Chrome 확장 프로그램입니다. 별도의 백엔드,
사용자 계정, 광고, 분석 도구 또는 원격 로그 수집 시스템을 운영하지 않습니다.
단, 사용자가 직접 여는 오류 신고 기능에는 Google Forms를 사용합니다.
...
사용자가 `Open PaperGlint` 또는 `Code & evidence`를 누르면 논문 제목, 일부 저자
...
직접 전송할 수 있습니다. PaperGlint는 쿠키 값을 읽거나 저장하지 않습니다.
...
`Report issue`를 누르면 Google Forms가 새 탭에서 열리며, 현재 논문 URL이
폼 주소의 매개변수로 Google에 전달됩니다. 이 전송은 폼을 제출하기 전에도
발생합니다. 폼을 제출하면 사용자가 작성한 신고 내용과 논문 URL을 개발자가
Google Forms 응답에서 열람하여 오류 재현과 지원에 사용합니다. 비밀번호,
API 키, 세션 쿠키 또는 민감한 개인정보를 작성하지 마세요.

신고 응답은 확장 프로그램의 캐시와 별도로 Google Forms에 보관되며,
확장 프로그램 제거로 삭제되지 않습니다. 확장 프로그램은 신고 응답의 자동 삭제를
수행하지 않습니다. 삭제를 원하면 아래 문의 링크에서 신고 시각과 논문 ID 등
해당 응답을 찾을 수 있는 최소 정보로 요청해 주세요. 민감한 신고 원문을
공개 이슈에 다시 게시하지 마세요. GitHub Issues에 직접 작성한 문의는 공개됩니다.
Google 서비스의 처리는 [Google 개인정보처리방침](https://policies.google.com/privacy)이
적용됩니다. Google Forms에 표시되는 로그인·이메일 수집 안내도 제출 전에 확인하세요.

## 제한된 사용

처리한 정보는 위에서 설명한 단일 사용자 기능을 제공하고 안정성을 유지하는 데만
사용합니다. 정보를 판매하거나 광고, 신용평가, 사용자 프로파일링에 사용하지 않고,
데이터 브로커 또는 광고 플랫폼에 제공하지 않습니다. 일반 논문 분석 결과를
개발자가 원격으로 열람하는 서버는 운영하지 않으며, 위에서 설명한 자발적 신고와
문의만 개발자가 지원 목적으로 열람합니다.

PaperGlint의 정보 사용은 Chrome Web Store 사용자 데이터 정책과 Limited Use
요구사항을 준수합니다.
